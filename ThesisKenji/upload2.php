...
// if everything is ok, try to upload file
    ...
    if (move_uploaded_file($Get["tmp_name"], $target_file)) {
        include("SQL.php");
        $query = "INSERT INTO image(ProductID, Image) 
        VALUES('".$ProductID."', '" . basename( $Get["name"]). "')";
...
if($X == 3){
    include("SQL.php");
    $query = "INSERT INTO product(ProductID, Name, Category, Price, Quantity, ShortD, LongD) 
                    VALUES('".$ProductID."','" .$Name. "', '" .$Category. "', '" .$Price. "', '" .$Quantity. "', '" .$ShortD. "', '" .$LongD. "')";
                        if($conn->query($query) == TRUE){
